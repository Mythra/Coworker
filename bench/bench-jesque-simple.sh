...
redis-cli -h localhost -n 0 -p 6379 ping
...
redis-cli -h localhost -n 0 -p 6379 FLUSHALL
echo "[+] Done."
...
java -jar $bench_dir/jesque/target/JesquePerfLoad.jar queue-jobs
echo "[+] Done."

start_time="$(date +%s)"
while true; do
  output="$(redis-cli -h localhost -n 0 --raw -p 6379 GET resque:stat:processed)"
  if [[ "$output" -lt "95000" ]]; then
    sleep 3
  fi
